...
public final class StdStats {
    ...
    public StdStats(float[] arr, int start, int end) {
        array = new double[end - start + 1];
        for(int i = start; i < end; i++) {
            array[i - start] = (double) arr[i];
        }
    }

    public StdStats(ArrayList<Double> arr) {
        array = new double[arr.size()];
